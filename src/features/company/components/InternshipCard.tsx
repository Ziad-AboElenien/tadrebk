    ...
    return (
      <Link href={`/internships/${internship._id}`}>
        <div className="group relative flex flex-col rounded-2xl bg-white/60 backdrop-blur-xl border border-white/70 shadow-[0_8px_32px_rgba(0,0,0,0.07)] hover:shadow-[0_16px_40px_rgba(16,185,129,0.15)] hover:-translate-y-0.5 transition-all duration-300 overflow-hidden">
            ...
            {internship.technicalSkills && internship.technicalSkills.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-3">
                {internship.technicalSkills.slice(0, 3).map((skill) => (
                  <span key={skill} className="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-700 border border-emerald-100">
                    {skill}
                  </span>
                ))}
          ...
          {/* Bottom action */}
          <div className="flex items-center justify-between px-5 py-3 border-t border-white/40 mt-auto">
            <span className="text-xs font-bold text-emerald-600 group-hover:text-emerald-700 transition-colors">
              View Details
            </span>
            <i className="fas fa-arrow-right text-[11px] text-emerald-500 group-hover:translate-x-0.5 transition-transform" />
          </div>
        </div>
      </Link>
    );
